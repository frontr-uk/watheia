...
import { SplitPane, Pane, Layout } from '../../base-ui.surfaces/split-pane/split-pane';
import useDimensions from 'react-use-dimensions';
import { useDebounce } from 'use-debounce';
import prismTheme from 'prism-react-renderer/themes/oceanicNext';
import { Error as errorClass } from '../../base-ui.input/error';
import styles from './react-playground.module.scss';
import { HoverSplitter } from '../../base-ui.surfaces/split-pane/hover-splitter';
...
export function Playground({ code, scope, style, className, ...rest }: PlaygroundProps) {
  ...
  const _code = useMemo(() => code?.toString().replace(/\n$/, '').trim(), [code]);
